...
def prep_for_ga(test_num, lig="ago"):
    if "09" in test_num and "07" not in test_num:
        ...
        val_df = pd.read_csv(f"../../data/row/validation_test_CAS/{test_num}_{lig}.csv", header=None)

    else:
        whole_df = pd.read_csv(f"../../data/processed/{test_num}/cas_sev.tsv", sep="\t", header=None)
        val_df = pd.read_csv(f"../../data/row/validation_test_CAS/{test_num}.csv", header=None)
...
def prep_validation(test_num, lig="ago"):
    ...
    if "09" in test_num and "07" not in test_num:
        pd.DataFrame(cas_not_in_whole).to_csv(f"../../data/processed/{test_num}_{lig}/cas_not_in_whole.tsv", sep="\t", header=None, index=False)
    else:
        pd.DataFrame(cas_not_in_whole).to_csv(f"../../data/processed/{test_num}/cas_not_in_whole.tsv", sep="\t", header=None, index=False)
